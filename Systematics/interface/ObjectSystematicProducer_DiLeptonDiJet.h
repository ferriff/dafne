#ifndef FLASHgg_ObjectSystematicProducer_DiLeptonDiJet_h
#define FLASHgg_ObjectSystematicProducer_DiLeptonDiJet_h
...
namespace flashgg {
    ...
    class ObjectSystematicProducer_DiLeptonDiJet : public edm::EDProducer
    {
    public:

        ObjectSystematicProducer_DiLeptonDiJet( const edm::ParameterSet & );
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    ObjectSystematicProducer_DiLeptonDiJet<flashgg_object, param_var, output_container>::ObjectSystematicProducer_DiLeptonDiJet( const ParameterSet &iConfig ) :
        globalVars_(iConfig),
        ObjectToken_( consumes<View<flashgg_object> >( iConfig.getParameter<InputTag>( "src" ) ) )
    {
        //        edm::Service<edm::RandomNumberGenerator> rng;
        //        if( ! rng.isAvailable() ) {
        //            throw cms::Exception( "Configuration" ) << "ObjectSystematicProducer_DiLeptonDiJet requires the RandomNumberGeneratorService  - please add to configuration";
        //        }
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    void ObjectSystematicProducer_DiLeptonDiJet<flashgg_object, param_var, output_container>::ApplyCorrections( flashgg_object &y,
            shared_ptr<BaseSystMethod<flashgg_object, param_var> > CorrToShift,
            param_var syst_shift )
    {
        float theWeight = 1.;
        //        std::cout << " 1d before 1d " << std::endl;
        //        std::cout << " In ObjectSystematicProducer_DiLeptonDiJet::ApplyCorrections pt m " << y.pt() << " " << y.mass() << std::endl;
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    void ObjectSystematicProducer_DiLeptonDiJet<flashgg_object, param_var, output_container>::ApplyNonCentralWeights( flashgg_object &y )
    {
        for( unsigned int ncorr = 0; ncorr < Corrections_.size(); ncorr++ ) {
            if( Corrections_.at( ncorr )->makesWeight() ) {
    ...
    template <typename flashgg_object, typename param_var, template <typename...> class output_container>
    void ObjectSystematicProducer_DiLeptonDiJet<flashgg_object, param_var, output_container>::produce( Event &evt, const EventSetup & setup )
    {
